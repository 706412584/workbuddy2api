...
func TestBackfillReasoningContentBothFields(t *testing.T) {
	...
	// 按 role 定位而非取 msgs[0]：管线前面还有 ensureSystemPrompt（首条非 system 时补一条），
	// 客户端没给 system 时索引 0 是补进来的 system，而非本条 assistant。
	var m map[string]any
	if err := json.Unmarshal(out, &m); err != nil {
		t.Fatal(err)
	}
	...
	for _, mm := range msgs {
		...
		found = true
		if r, ok := msg["reasoning"].(string); !ok || r != "t" {
			t.Errorf("reasoning 字段被改动: %v (out=%s)", msg, out)
		}
	}
	if !found {
		t.Errorf("未找到 assistant 消息 (out=%s)", out)
	}
}
