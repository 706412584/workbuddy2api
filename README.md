...
| 能力 | 说明 |
|---|---|
| 🔑 **OAuth 一键登录** | `login.sh` 设备授权流程（无 PKCE），自动落盘凭证并重启容器；支持国内版与国外版，**global 账号自动完成激活**（免开 IDE） |
...
| 🖥️ **管理面板** | 与 API 同端口的网页面板（`/__admin/*`，仅本机可访问）：账号 / 密钥 / 日志 / 统计，改动**进程内热重载**，不重启不断流 |
...
| ⏰ **定时任务** | 签到（09/21 点，global 区自动跳过）+ 活跃上报（10 点，点亮连登 / 解锁领养 + streak 自检）+ 猫猫旅行（09/21 点，独立排程）+ token 保活（22 点），四类各自独立开关 |
| ⚡ **流式 + 非流式** | 上游 SSE 逐帧规范化透传；出站强制 `stream:true`，非流式由本地聚合为单响应 |
| 🧠 **推理模型兼容** | DeepSeek 思维链注入（`thinking.type=enabled` + 默认档）、`reasoning_content` 多轮回填、工具调用（`tool_calls`）按 index 合并、effort 档位自动降级；**思考死循环观测**（命中特征打 `thinking_loop` 日志） |
...
| `server.max_body_mb` | `32` | 请求体大小上限（MB，0 / 负数启动报错）。超限返回 **413 `request_body_too_large`**，不再静默截断喂给上游（多图/超长上下文会话请调大） |
| `upstream.user_agent` | 空 | 出站 User-Agent 覆盖。空 = 默认官方 WorkBuddy 三段式 `WorkBuddy/<ver> WorkBuddy/<ver> CLI/<ver>` |
| `upstream.client_version` / `cli_version` | 空 | 出站 UA 与 `X-IDE-Version` 的版本段；空 = 内置默认（对齐官方分发包） |
| `upstream.device_token` / `device_token_file` | 空 | `X-Device-Token` 设备风控头来源（每号 auths 文件 > 本项 > 文件兜底）；空 = 不注入 |
| `upstream.client_name` | 空 | 用量归属头 `X-Product`/`X-IDE-*` 取值；空 = 仅 `X-Product=SaaS`（旧行为） |
| `upstream.passthrough_ip` | `false` | 是否把客户端 IP（`X-Forwarded-For` 首段）透传给上游 |
...
`WB2A_LISTEN` · `WB2A_API_KEY` · `WB2A_AUTH_DIR` · `WB2A_STATE_FILE` · `WB2A_MAX_BODY_MB` · `WB2A_SOFT_RATE`(duration) · `WB2A_SOFT_RATE_MAX`(duration) · `WB2A_TIMEOUT_SECONDS` · `WB2A_HEADER_TIMEOUT_SECONDS` · `WB2A_IDLE_TIMEOUT_SECONDS` · `WB2A_USER_AGENT` · `WB2A_CLIENT_VERSION` · `WB2A_CLI_VERSION` · `WB2A_DEVICE_TOKEN` · `WB2A_DEVICE_TOKEN_FILE` · `WB2A_CLIENT_NAME` · `WB2A_PASSTHROUGH_IP`(bool) · `WB2A_SANITIZE_FINGERPRINTS`(bool) · `WB2A_PROMPT_MODE` · `WB2A_PROMPT_FILE`
...
| `GET /status` | Bearer（`api_key` 非空时） | 账号状态汇总 + 每账号详情（积分/冷却/熔断/在途/粘性/区域；disabled 账号透出 `disabled_reason`） |
...
出站请求统一携带官方 WorkBuddy 三段式 UA `WorkBuddy/<ver> WorkBuddy/<ver> CLI/<ver>`（可被 `upstream.user_agent` 覆盖）；聊天请求带账号头（`X-User-Id` 等），**永不携带 `X-Refresh-Token`**（该头只出现在 token 刷新请求）。
...
| 请求体上限默认 32 MB | `cmd/server/config.go`（`Default()`）；413 判定与返回 `internal/server/handler.go`（`readBodyOrFail`） |
